  ...
  outputs = { self, nixpkgs, flake-utils, nix2container }: flake-utils.lib.eachDefaultSystem (
      system:
        let 
            ...
            globalStackConfigFile = with pkgs; pkgs.writeTextDir ".stack/config.yaml" ''
                # stackage
            ...
            globalStackConfigHintFile = with pkgs; pkgs.writeTextDir ".stack/pantry/global-hints-cache.yaml" ''
                https://mirrors.tuna.tsinghua.edu.cn/github-raw/fpco/stackage-content/master/stack/global-hints.yaml
            '';

            global-stack-config = with pkgs; buildEnv {
                name = "global-stack-config";
                pathsToLink = [ "/.stack" "/.stack/pantry" ];
                paths = [ globalStackConfigFile globalStackConfigHintFile ];
